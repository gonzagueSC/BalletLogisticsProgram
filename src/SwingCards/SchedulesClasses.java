...
public class SchedulesClasses extends Panel {
	...
	public SchedulesClasses(SchedulesMainView Panel) {
	...
	public void Update() {
		...
		String[] allRehearsals = SchedulesModule.getAllClasses();
		...
		for (int i = 0; i < allRehearsals.length; i++) {
			...
			Button Rehearsal = new Button(RehearsalName.split(", ")[0] + ": " + RehearsalName.split(", ")[1],
					(e -> DialogGenerator.newClassEditor(this, RehearsalName)), new Dimension(BUTTONWIDTH, BUTTONHEIGHT),
			...
			Button Remove = new Button("Remove", (e -> {
				...
				SchedulesModule.removeClass(RehearsalName);
				Update();
			...
			Button Pause = new Button("Pause", (e -> {
				...
				DialogGenerator.createPauseClassDialog(this, RehearsalName);
				Update();

			}), new Dimension(200, 80), BUTTONX + 670, i * (BUTTONHEIGHT + BUTTONGAP) + BUTTONGAP + 10, Color.LIGHT_GRAY,
					BUTTONARCRAD - 5, BUTTONFONTSIZE - 10);
		...
		Button newRehearsal = new Button("+ Add Classes", (e -> DialogGenerator.newClassDialog(this)), BOTTOMADDDim, 1000, 515,
